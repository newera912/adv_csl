...
class Task_inference(object):
    def __init__(self, omega, b, X_b, y_t, Y, X, edge_up_nns,edge_down_nns, omega_0, R, dict_paths, psl, approx, report_stat):
        self.omega = omega
        self.b=b
        self.X_b=X_b
        self.y_t = y_t
    ...
    def __call__(self):
        ...
        p_t, b_t= admm(self.omega,self.b,self.X_b, self.y_t, self.Y, self.X, self.edge_up_nns,self. edge_down_nns, self.omega_0, self.R, self.dict_paths, self.psl, self.approx, self.report_stat)
        return p_t, b_t
...
def inference(omega_y, omega_0, y, X_b, X, Y, T, edge_up_nns, edge_down_nns, R, dict_paths, logging, psl = False, approx = False, init_alpha_beta = (1, 1), report_stat = False):
    ...
    for iter in range(5):
        ...
        for t in range(T):
            ...
            tasks.put(Task_inference(omega, b, X_b, y_t, Y, X, edge_up_nns,edge_down_nns, omega_0, R, dict_paths, psl, approx, report_stat))
            num_jobs+=1.0
...
def inference_apdm_format(V, E, Obs, Omega, b, X_b, E_X, logging, psl = False, approx = True, init_alpha_beta = (1, 1), report_stat = False):
    ...
    pred_omega_x, pred_b_y = inference(omega_y, omega_0, y, X_b, X, Y, T, {}, edge_down_nns, R, dict_paths, logging, psl, approx, init_alpha_beta, report_stat)
...
def admm(omega, b_init, X_b, y_t, Y, X, edge_up_nns, edge_down_nns, omega_0, R, dict_paths, psl = False, approx = False, report_stat = False):
    ...
    for iter in range(maxiter):
        ...
        for k in xrange(K):
            ...
            R_p_hat[k] = R_p[k] - kappa * R_lambda[k]
            # print "R_p", R_p
