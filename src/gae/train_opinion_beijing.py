...
from gae.input_data import load_data_beijing
from gae.model import GCNModelAE, GCNModelVAE
from gae.preprocessing import preprocess_graph, construct_feed_dict, sparse_to_tuple, mask_test_edge_opinion_beijing
...
flags.DEFINE_float('alpha_0', 7, 'prior of Beta distribution.')
flags.DEFINE_float('beta_0', 14, 'prior of Beta distribution.')
...
flags.DEFINE_float('test_num', 0.3, 'test number of dataset.')
...
adj, features = load_data_beijing()
...
# adj = adj_train
adj_train = adj
y_train_belief, y_test_belief, y_train_un, y_test_un, train_mask, test_mask, omega_test = mask_test_edge_opinion_beijing(FLAGS.test_num)
...
placeholders = {
    ...
    'labels_mask': tf.placeholder(tf.int32, shape=(None, train_mask.shape[1])),
...
for epoch in range(FLAGS.epochs):
    ...
    avg_cost = outs[1]
    np.save("./data/tem_b", outs[2])
    np.save("./data/tem_u", outs[3])
    # avg_accuracy = outs[2]
