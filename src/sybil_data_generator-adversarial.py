...
from gen_sybils_adv_example import inference_apdm_format as inference_adv_coi
from gen_sybils_adv_example_csl import inference_apdm_format as inference_csl
...
class Task_generate_strusture(object): #dataset,attack_edge,V, E, Obs, T, test_ratio, swap_ratio,alpha
    ...
    def __call__(self):
        ...
        for i, v0 in enumerate(self.target_nodes):
            ...
            if len(self.adj_list[v0]) < self.perturbation:
                for u in self.adj_list[v0][:-1]:
                    ...
                    if not E0.has_key((v0, u)) and not E0.has_key(((u, v0))):
                        E0[(v0, u)] = 1.0
                        E0[(u, v0)] = 1.0
                        add_remove.append(1)
                    else:
                    ...
                    for t in range(0, self.T):
                        if random.random() < 0.5:
                            Obs[u][t] = self.Obs0[u][t] - 0.2 if self.Obs0[u][t] - 0.2 > 0 else self.Obs0[u][t] + 0.2  # clip between [0,1]
                        else:
                            Obs[u][t] = self.Obs0[u][t] + 0.2 if self.Obs0[u][t] + 0.2 < 1.0 else self.Obs0[u][t] - 0.2
                    add_remove.append(0)
                ...
                for e in candidate_edges:
                    ...
                    E0.pop(e, None)
                    E0.pop((e[1], e[0]), None)
                    p_adv = gen_adv_exmaple_structure(self.V, E0, Obs, self.E_X, v0)

                    if p_adv == self.Obs0[v0][0]:  # no changes
                        print(self.perturbation,num_perturbtion,p_adv, self.Obs0[v0][0], "not changed...")
                        E0[e] = 1.0
                        E0[(e[1], e[0])] = 1.0
                    else:
                        print(self.perturbation,num_perturbtion,p_adv, self.Obs0[v0][0], "changed...")
                        u = e[0] if e[0] != v0 else e[1]
                        for t in range(0, self.T):
                            if random.random() < 0.5:
                                Obs[u][t] = self.Obs0[u][t] - 0.2 if self.Obs0[u][t] - 0.2 > 0 else self.Obs0[u][t] + 0.2  # clip between [0,1]
                ...
                if num_perturbtion < self.perturbation:
                    for ee in candidate_edges:
                        if num_perturbtion >= self.perturbation:
                            break
                        if not E0.has_key(ee) and not E0.has_key((ee[1], ee[0])):
                            E0[ee] = 1.0
                            E0[(ee[1], ee[0])] = 1.0
                        else:
                            E0.pop(ee, None)
                            E0.pop((ee[1], ee[0]),None)
                        u = ee[0] if ee[0] != v0 else ee[1]
                        for t in range(0, self.T):
                            if random.random() < 0.5:
                                Obs[u][t] = self.Obs0[u][t] - 0.2 if self.Obs0[u][t] - 0.2 > 0 else self.Obs0[u][t] + 0.2  # clip between [0,1]
                            else:
        ...
        pkl_file = open(self.fout, 'wb')
        pickle.dump([self.V, E0, Obs, self.E_X, self.target_nodes], pkl_file)
        pkl_file.close()
        return
...
def generate_structure(dataset,attack_edge,V, E,Obs, T,test_ratio,swap_ratio,real_i,out_folder,node_degree,adj_list):
    ...
    num_consumers = 12  # We only use 5 cores.
