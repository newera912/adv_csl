...
class Task_inference(object):
    ...
    def __call__(self):
        ...
        p_t = admm(self.omega, self.y_t, self.Y, self.X, self.edge_up_nns,self. edge_down_nns, self.p0, self.R, self.dict_paths, self.psl, self.approx, self.report_stat)
        return p_t
...
def inference_apdm_format(V, E, Obs, Omega, E_X, logging, psl = False, approx = True, init_alpha_beta = (1, 1), report_stat = False):
    ...
    omega_x = inference(omega_y, y, Y, X, T, edge_up_nns, edge_down_nns, p0, R, dict_paths, x, logging, psl, approx, init_alpha_beta, report_stat)
