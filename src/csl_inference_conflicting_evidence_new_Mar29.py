...
"""
def inference_apdm_format(V, E, Obs, Omega, b, X_b, X, logging, psl = False, approx = True, init_alpha_beta = (1, 1), report_stat = False):
    ...
    X_b = {edge_2_id[e]: 1 for e in X_b}
...
def calc_initial_p(dict_paths, y_t, edge_down_nns, X_b, X, Y, cnt_E, p0, b_init):
    ...
    for e in Y:
        p[e] = y_t[e]
        if dict_paths.has_key(e) and e in X_b:  #if a edge in Y and also as an atom in head part of any rule
            n_pos = 0
            n_neg = 0
            for (e1,e2) in dict_paths[e]:
                if e1 in X or e2 in X: continue  #if e as a atom in the head of a rule,
                if p[e1] * p[e2] > 0:
                    n_pos += 1
                else:
                    n_neg += 1
            if n_pos + n_neg == 1: continue
        ...
        for k in range(K):
            ...
            if lk(k, X_b, R[k], R_p_hat[k],y_edge[k])>0:
                ...
                if lk(k, X_b, R[k], R_p_hat[k], y_edge[k]) < 0:
                    ...
                    R_p_hat[k] = normalize(Proj_lk(k, X_b, R[k], list_minus(R_p[k], list_dot_scalar(R_lambda[k], kappa)), R_p_hat[k], y_edge[k]))
                    # print "projection after: ", lk(k, R_p_hat[k],y_edge[k]), R_p_hat[k]
...
def get_comp(k, X_b, R_k, rule, y_edge):
    ...
    for i in range(3):
        if R_k[i] not in X_b: # set X corresponding b_y to 0;
            c[i*2 + 1] = 0
...
def testcase_11_0():
    logging = Log('testcase_13.txt')
    print inspect.stack()[0][3]
    V = [0, 1, 2, 3,4]
    E = [(0,1),(0,2),(0,3),(2,1),(1,3),(2,3)]
    # E = [(2, 1), (1, 3), (2, 3)]
    X = [(0,1)]
    Y = [e for e in E if e not in X]
    T = 1
    ...
    Omega = estimate_omega_x([{e: Obs[e][t] for e in Obs} for t in range(T)], E)
    b = {e: 1 if e in true_conflict else 0 for e in E}
    print "initial Omegas",Omega
    X_b = Y
    prob_mse, u_mse, prob_relative_mse, u_relative_mse, accuracy, recall_congested, recall_uncongested, b_f_measure = \
        evaluate(V, E, Obs, Omega, X, Y, X_b, b, logging, psl=True, approx=False, init_alpha_beta=(1, 1),
                 report_stat=False)
...
def testcase_11_01():
    ...
    print inspect.stack()[0][3]
    V = [0, 1, 2, 3,4]
    E = [(0,1),(0,2),(0,3),(0,4),(2,1),(1,3),(2,3),(3,4)]
    X = [(0,1),(0,4)]
    Y = [e for e in E if e not in X]
    T = 1
    zeros = np.zeros(T)
...
#     # print "b", b
#     prob_mse, u_mse, prob_relative_mse, u_relative_mse, accuracy, recall_congested, recall_uncongested, b_f_measure = \
#         evaluate(V, E, Obs, Omega, X, Y, b, logging, psl = True, approx = False, init_alpha_beta = (1, 1), report_stat =False)
#
#
#     V = [0,1,2,3,4]
#     E = [(0, 1), (0, 2), (0, 3), (0, 4), (1, 3), (2, 1), (3, 4)]
