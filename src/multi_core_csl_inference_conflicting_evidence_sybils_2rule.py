...
def admm(omega, b, y_t, Y, X, node_nns, omega_0, R, psl=False, approx=False, report_stat=False):
    ...
    K = len(R)
    t0 = time.time()
    p_init, b_init = calc_initial_node_p1(y_t, node_nns, X, Y, cnt_V, 0.5, b)
    t1 = time.time()
    ...
    for iter in range(maxiter):
        ...
        for k in range(K):
            ...
            if lk(k, Y, R[k], R_p_hat[k], y_edge[k]) > 0:
                _, c = get_comp(k, Y, R[k], R_p_hat[k], y_edge[k])
                c = np.array(c)
                ...
                if lk(k, Y, R[k], R_p_hat[k], y_edge[k]) < 0:
                    proj_num+=1
                    # print "proj k......"
                    # print "projection: ", lk(R_p_hat[k],y_edge[k]), R_p_hat[k]  #(x, rule, y_edge)
                    R_p_hat[k] = normalize(Proj_lk2(k, Y, R[k], R_p[k] - kappa * R_lambda[k], R_p_hat[k], y_edge[k]))
...
def lk(k, Y, R_k, rule, y_edge):
    const, c = get_comp(k, Y, R_k, rule, y_edge)
...
def get_comp(k, Y, R_k, rule, y_edge):
...
def Proj_lk2(k, Y, R_k,x, rule, y_edge):
    const, c = get_comp(k, Y, R_k, rule, y_edge)
    c_part = []
    x_part = []
    idx_list = []
    cnt=0
    for i in range(2):
        if Y.has_key(R_k[i]):#if all edges are target edges
            cnt += 1
    # try:
