...
class Task_inference(object):
    def __init__(self, omega, b, X_b, y_t, Y, X, edge_up_nns, edge_down_nns, omega_0, R, dict_paths, psl, approx,
                 report_stat):
        self.omega = omega
        self.b = b
        self.X_b = X_b
    ...
    def __call__(self):
        ...
        p_t, b_t = admm(self.omega, self.b, self.X_b, self.y_t, self.Y, self.X, self.edge_up_nns, self.edge_down_nns,
                        self.omega_0, self.R, self.dict_paths, self.psl, self.approx, self.report_stat)
        return p_t, b_t
...
class Task_admm(object):
    def __init__(self, X_b, R, R_p, R_p_hat, R_lambda, y_edge, start_k, end_k, kappa):
        self.X_b = X_b
        self.R = R
        self.R_p = R_p
        self.R_p_hat = R_p_hat
    ...
    def __call__(self):
        ...
        # admm(omega, b, X_b, y_t, Y, X, edge_up_nns,edge_down_nns, omega_0, R, dict_paths, psl, approx, report_stat)
        weight = 1.0
        for k in range(len(self.R_p)):

            self.R_lambda[k] = list_sum(self.R_lambda[k],
                                        list_dot_scalar(list_minus(self.R_p_hat[k], self.R_p[k]), 1 / self.kappa))
            # update copy variables
            self.R_p_hat[k] = normalize(list_minus(self.R_p[k], list_dot_scalar(self.R_lambda[k], self.kappa)))
            # print "R_p", R_p
            if lk(k, self.X_b, self.R[k], self.R_p_hat[k], self.y_edge[k]) > 0:
                _, c = get_comp(k, self.X_b, self.R[k], self.R_p_hat[k], self.y_edge[k])
                k_p_hat = normalize(
                    list_minus(self.R_p[k], list_sum(list_dot_scalar(self.R_lambda[k], self.kappa),
                                                     list_dot_scalar(c, weight * self.kappa))))
                # print "k_p_hat", k_p_hat
                for i, edge_i_val in enumerate(self.y_edge[k]):
                    if self.y_edge[k][i] == 1:  # update b_y
                        self.R_p_hat[k][i * 2 + 1] = k_p_hat[i * 2 + 1]
                    else:  # update p_x
                        self.R_p_hat[k][i * 2] = k_p_hat[i * 2]
                # print k, R_p_hat[k],y_edge[k], lk(k, R_p_hat[k], y_edge[k])
                if lk(k, self.X_b, self.R[k], self.R_p_hat[k], self.y_edge[k]) < 0:
                    # print "projection before: ", lk(k, R_p_hat[k],y_edge[k]), R_p_hat[k]
                    self.R_p_hat[k] = normalize(
                        Proj_lk(k, self.X_b, self.R[k],
                                list_minus(self.R_p[k], list_dot_scalar(self.R_lambda[k], self.kappa)), self.R_p_hat[k],
                                self.y_edge[k]))
                # print "projection after: ", lk(k, R_p_hat[k],y_edge[k]), R_p_hat[k]
            # print "R_p_hat[k] updated: ", R_p_hat[k]
        return self.R_p_hat, self.R_lambda, self.start_k, self.end_k
...
def inference(omega_y, omega_0, y, X_b, X, Y, T, edge_up_nns, edge_down_nns, R, dict_paths, logging, psl=False,
              approx=False, init_alpha_beta=(1, 1), report_stat=False):
    ...
    for iter in range(5):
        ...
        for t in range(T):
            ...
            p_t, b_t = admm(omega, b, X_b, y_t, Y, X, edge_up_nns, edge_down_nns, omega_0, R, dict_paths, psl, approx,
                            report_stat)
            ps.append(p_t)
            bs.append(b_t)
...
def inference_apdm_format(V, E, Obs, Omega, b, X_b, E_X, logging, psl=False, approx=True, init_alpha_beta=(1, 1),
    ...
    pred_omega_x, pred_b_y = inference(omega_y, omega_0, y, X_b, X, Y, T, {}, edge_down_nns, R, dict_paths, logging,
...
def generate_local_variables2(p_init, b_init, X_b, y_t, edge_down_nns, X, Y, cnt_E, R):
    K = len(R)
    R_p = []
...
def generate_local_variables(p_init, b_init, X_b, y_t, edge_down_nns, X, Y, cnt_E, R):
    K = len(R)
    R_p = np.zeros((K,6))
    R_p_hat = np.zeros((K,6))  # R_z is a vector of lists of copied variables in R_p and each of the list relates to a specific rule.  R_z[i][e] is a copied variable of R_p[i][e]
...
def admm(omega, b_init, X_b, y_t, Y, X, edge_up_nns, edge_down_nns, omega_0, R, dict_paths, psl=False, approx=False,
         report_stat=False):
    # print "b_init", b_init
    weight = 1.0
    epsilon = 0.01
    ...
    for iter in range(maxiter):
        ...
        # Start consumers
        num_consumers = 20  # We only use 5 cores.
        ...
        num_jobs = 0
        t0 = time.time()
        m = int(np.ceil(float(len(R) / 3.0) / num_consumers))
        for (start_k, end_k) in chunks(int(len(R) / 3.0), m):
            start_k = 3 * start_k
            end_k = 3 * end_k
            tasks.put(Task_admm(X_b, R[start_k:end_k], R_p[start_k:end_k], R_p_hat[start_k:end_k],
                                R_lambda[start_k:end_k], y_edge[start_k:end_k], start_k, end_k, kappa))
            num_jobs += 1.0

            # update lambda variables
        t1 = time.time()
        print "Task Distribute time:", t1 - t0
        for i in xrange(num_consumers):
            tasks.put(None)

        while num_jobs:
            R_p_hat_i, R_lambda_i, start_k, end_k = results.get()
            for k in range(len(R_p_hat_i)):
                R_lambda[start_k + k] = R_lambda_i[k]
                R_p_hat[start_k + k] = R_p_hat_i[k]
            num_jobs -= 1.0
        t2 = time.time()

        print "Prob and lambda updated resutlts collection is done...", t2 - t1
        t_2=time.time()
