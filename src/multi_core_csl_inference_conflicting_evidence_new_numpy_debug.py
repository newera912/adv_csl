...
class Task_inference(object):
    def __init__(self, omega, b, X_b, y_t, Y, X, edge_up_nns,edge_down_nns, omega_0, R, dict_paths,x_on_body, psl, approx, report_stat):
        self.omega = omega
        self.b=b
        self.X_b=X_b
        self.y_t = y_t
    ...
    def __call__(self):
        ...
        p_t, b_t= admm(self.omega,self.b,self.X_b, self.y_t, self.Y, self.X, self.edge_up_nns,self. edge_down_nns, self.omega_0, self.R, self.dict_paths,self.x_on_body, self.psl, self.approx, self.report_stat)
        return p_t, b_t
    ...
    for e in Y:
        ...
        if dict_paths.has_key(e) and X_b.has_key(e):
            ...
            if (n_pos - n_neg > 0 and p[e] == 0) or (n_pos - n_neg < 0 and p[e] > 0):
                """b_init[e] = 1.0"""
                b_init[e] =1.0 #random.random()#1.0
            else:
                b_init[e] =0.0 #random.random()#0.0
    ...
    for e in X:
        ...
        if dict_paths.has_key(e):
            ...
            if conf > 0: #if (1,1) count is greater than (_,1),(1,_),(_,0) and (0,_) counts than set to 1
                """p[e] = 1.0"""
                p[e] = 1.0  # round(random.random(),2) #1.0
            else:
                p[e] =0.0   # round(random.random(),2) #0.0
        elif x_on_body.has_key(e):
...
b_t:
"""
def admm(omega, b_init, X_b, y_t, Y, X, edge_up_nns, edge_down_nns, omega_0, R, dict_paths,x_on_body, psl = False, approx = False, report_stat = False):
    # print "b_init", b_init
    # print "admm start...."
    weight = 1.0
    epsilon = 0.01
    ...
    K = len(R)
    # t0=time.time()
    p_init, b_init = calc_initial_p(dict_paths,x_on_body, y_t, edge_down_nns, X_b, X, Y, cnt_E, 0.5, b_init)
    # t1=time.time()
