...
from data.synthetic_opinion import *
...
def mask_test_node_sybils(fileName, T):
    ...
    [V, E, _, _,_] = pickle.load(pkl_file)
    ad_m = np.zeros([len(V), len(V)])
    for (v1,v2) in E:
        ad_m[v1][v2] = int(1)
    print("|V|:{} Adj:{} ".format(len(V),len(ad_m)))
    # features = sp.identity(len(ad_m))
    # adj_mask = np.reshape(adj_mask, [-1])
    adj = sp.csr_matrix(ad_m)
    belief, uncertain, test_index,target_nodes = rb_data.get_epinion_data_sybils(fileName,T)
    print("|b|:{} |u|:{} |test_index|:{} ".format(len(belief), len(uncertain),len(test_index)))
    ...
    target_mask=np.zeros_like(belief, dtype=bool)
    for i in range(len(test_mask)):
        ...
        if i in test_index:
            ...
            train_mask[i] = True
    return adj,y_train_belief, y_test_belief, y_train_un, y_test_un, train_mask, test_mask,target_mask, omega, a0, b0

def mask_test_edge_opinion(fileName, T):
    pkl_file = open(fileName, 'rb')
    # pkl_file = open("/network/rit/lab/ceashpc/fenglab/baojian/git/subjectiveLogic/eopinion/data/trust-analysis2/nodes-7500-T-6-rate-0.2-testratio-0.2-swaprate-0.05-realization-0-data-X.pkl")
    [_, E, _, _, _] = pickle.load(pkl_file)
    ...
    # adj_mask = np.reshape(adj_mask, [-1])
    adj = sp.csr_matrix(ad_m)
    belief, uncertain, test_index = rb_data.get_epinion_data(fileName, T)
    ...
    return adj,y_train_belief, y_test_belief, y_train_un, y_test_un, train_mask, test_mask, omega, a0, b0


def mask_test_edge_epinion(fileName, T):
    pkl_file = open(fileName, 'rb')
    # pkl_file = open("/network/rit/lab/ceashpc/fenglab/baojian/git/subjectiveLogic/eopinion/data/trust-analysis2/nodes-7500-T-6-rate-0.2-testratio-0.2-swaprate-0.05-realization-0-data-X.pkl")
    [_, E, _, _,_] = pickle.load(pkl_file)
    ad_m = np.zeros([len(E), len(E)])
