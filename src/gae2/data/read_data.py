...
def load_data_bigdata(week, test_num, hour):
    random.seed(132)
    # adj_n = get_adjacency_matrix()
...
def load_data_direct(week, test_num, hour):
    random.seed(132)
    # adj_n = get_adjacency_matrix()
    adj_n = np.load("/network/rit/lab/ceashpc/xujiang/GAT/GAT1/data/traffic_data/adjacency_matrix_dc_direct.npy")
    features_ = np.load("/network/rit/lab/ceashpc/xujiang/GAT/GAT1/data/traffic_data/direct_feature_dc_0.8_6hours_Fr.npy")
    feature_hour = features_[hour]
    feature_edge_i = feature_hour[:, week]
    test_index = random.sample(range(len(feature_edge_i)), test_num)
    label, feature_n = get_hotvalue_bigdata(feature_edge_i)
...
def load_data_eopinion(week):
    random.seed(132)
    # adj_n = get_adjacency_matrix()
    adj_n = np.load("/network/rit/home/xz381633/traffic_deep/input_data/adjacency_matrix_eopinion_10000.npy")
    feat_ = np.load("/network/rit/lab/ceashpc/xujiang/eopinion/graph_10000_test_0.2.npz")
    feature_hour = feat_["arr_0"]
    # feature_hour = features_[hour]
    feature_edge_i = feature_hour[0][:, week]
    # test_index = random.sample(range(len(feature_edge_i)), test_num)
    test_index = feat_["arr_1"]
    label, feature_n = get_hotvalue_bigdata(feature_edge_i)
...
def same_size_2(feat):
    if len(feat) == 7:
        feat = [feat[0] + feat[1] + feat[2] + 0.5 * feat[3], feat[4] + feat[5] + feat[6] + 0.5 * feat[3]] * (2 / 7)
    elif len(feat) == 6:
        feat = [feat[0] + feat[1] + feat[2], feat[4] + feat[5] + feat[3]] * (2 / 6)
    elif len(feat) == 5:
        feat = [feat[0] + feat[1] + 0.5 * feat[2], feat[3] + feat[4] + 0.5 * feat[2]] * (2 / 5)
    elif len(feat) == 4:
        feat = [feat[0] + feat[1], feat[2] + feat[3]] * (2 / 4)
    elif len(feat) == 3:
        feat = [feat[0] + 0.5 * feat[1], feat[2] + 0.5 * feat[1]] * (2 / 3)
    elif len(feat) == 1:
        feat = [feat[0] / 2.0, feat[0] / 2.0]
    elif len(feat) == 0:
        feat = [0.0, 0.0]
    return feat


def same_size_4(feat):
    if len(feat) == 7:
        feat = [feat[0] + 0.5 * feat[1], feat[2] + 0.5 * (feat[1] + feat[3]), feat[4] + 0.5 * (feat[3] + feat[5]),
                feat[6] + 0.5 * feat[5]] * (4 / 7)
    elif len(feat) == 6:
        feat = [feat[0] + 0.5 * feat[1], feat[2] + 0.5 * feat[1], feat[3] + 0.5 * feat[4], feat[5] + 0.5 * feat[4]] * (
                4 / 6)
    elif len(feat) == 5:
        feat = [feat[0] + 0.25 * feat[2], feat[2] + 0.25 * feat[2], feat[3] + 0.25 * feat[2],
                feat[4] + 0.25 * feat[2]] * (4 / 5)
    elif len(feat) == 3:
        feat = [0.5 * feat[0] + 0.25 * feat[1], 0.5 * feat[2] + 0.25 * feat[1], 0.5 * feat[0] + 0.25 * feat[1],
                0.5 * feat[2] + 0.25 * feat[1]]
    elif len(feat) == 2:
        feat = [0.5 * feat[0], 0.5 * feat[1], 0.5 * feat[0], 0.5 * feat[1]]
    elif len(feat) == 1:
        feat = [feat[0] / 4.0, feat[0] / 4.0, feat[0] / 4.0, feat[0] / 4.0]
    elif len(feat) == 0:
        feat = [0.0, 0.0, 0.0, 0.0]
    return feat


def generate_neighbor_feature():
    feature_neigh_u, feature_neigh_v = get_neighbor_feat(which_feat=0)
    centroid = get_cluster()
    feat_u = []
    feat_v = []
    for p in range(len(feature_neigh_u)):
        feat_u_p = []
        feat_v_p = []
        if p == 65:
            pass
        if centroid[p] == 0:  # 2*2
            feat_u_p = same_size_2(feature_neigh_u[p])
            feat_v_p = same_size_2(feature_neigh_v[p])
        elif centroid[p] == 1:  # 2*4
            feat_u_p = same_size_2(feature_neigh_u[p])
            feat_v_p = same_size_4(feature_neigh_v[p])
        elif centroid[p] == 2:  # 4*2
            feat_u_p = same_size_4(feature_neigh_u[p])
            feat_v_p = same_size_2(feature_neigh_v[p])
        feat_u.append(feat_u_p)
        feat_v.append(feat_v_p)
    print(1)
    return
...
if __name__ == '__main__':
    # save_all_feature()
    # ff, _ = get_features(feature_path_)
    # np.save("/network/rit/home/xz381633/traffic_deep/input_data/features_hour14_week2_25.npy", ff)
    # bb = np.load("/network/rit/home/xz381633/traffic_deep/input_data/features_hour14_week2_25.npy")
    # for i in range(35):
    #     item = bb[:, i]
    #     print(Counter(item))
    # save_all_feature()
    # aa = np.load("/network/rit/home/xz381633/traffic_deep/input_data/features_all_25_ph.npy")
    # get_neighbor()
    print(1)
