...
class OptimizerVAE(object):
    def __init__(self, preds, labels, model, num_nodes, pos_weight, norm, label_b, label_un, mask,target_mask, omega_t):
        preds_sub = preds
        labels_sub = labels
        ...
        #                                    tf.cast(labels_sub, tf.int32))
        # self.accuracy = tf.reduce_mean(tf.cast(self.correct_prediction, tf.float32))
        self.test_mse = masked_mse_abs(model.belief, label_b, target_mask) + masked_mse_abs(model.uncertain, label_un, target_mask)
        self.omega_mse = masked_mse_abs(model.omega, omega_t, target_mask)

        self.belief=model.belief
        self.uncertain=model.uncertain