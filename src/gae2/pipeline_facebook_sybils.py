...
<<<<<<< HEAD
                            fileName = dataroot +adv_type+ "/{}/{}-attackedges-{}-T-{}-testratio-{}-swap_ratio-{}-perturbation-{}-realization-{}-data-X20-SF.pkl".format(
                                    "facebook","facebook", attack_edge, T, test_ratio, swap_ratio, perturbation, real_i)
                            outf = '../../output/sybils/{}_results-server-June4-SF-{}.json'.format("GCN-VAE", adv_type)
=======
                            fileName = dataroot +adv_type+ "/{}/{}-attackedges-{}-T-{}-testratio-{}-swap_ratio-{}-perturbation-{}-realization-{}-data-X20-SF2.pkl".format(
                                    "facebook","facebook", attack_edge, T, test_ratio, swap_ratio, perturbation, real_i)
                            outf = '../../output/sybils/{}_results-server-June4-TN20-SF2-{}.json'.format("GCN-VAE", adv_type)
>>>>>>> 58829a7d39378f4b7dfe53cb4fc8d64f19c8f321
                            ...
                            with tf.name_scope('optimizer'):
                                if model_str == 'gcn_ae':
                                    ...
                                    opt = OptimizerVAE(preds=model.reconstructions,
                                                       ...
                                                       mask=placeholders['labels_mask'], target_mask=placeholders['target_mask'], omega_t=placeholders['omega_t'],)
                            ...
                            for k in range(1):
                                ...
                                _,y_train_belief, y_test_belief, y_train_un, y_test_un, train_mask, test_mask,target_mask, omega_test, alpha_0, beta_0 = mask_test_node_sybils(fileName, T=FLAGS.T)
                                bf_mse_one = []
                                b_mse_one = []
                                u_mse_one = []
                                t1 = time.time()
                                for epoch in range(FLAGS.epochs):
                                    ...
                                    feed_dict = construct_feed_dict(adj_norm, adj_label, features, placeholders, y_train_belief, y_train_un,
                                                                    train_mask,target_mask, omega_test, alpha_0, beta_0)
                                    ...
                                    if np.mod(epoch + 1, 10) == 0:
                                        feed_dict = construct_feed_dict(adj_norm, adj_label, features, placeholders, y_test_belief, y_test_un,
                                                                        test_mask,target_mask, omega_test, alpha_0, beta_0)
                                ...
                                result_ = {'dataset':"facebook",'attack_edge':attack_edge,'adv_type':adv_type,
                                           ...
                                           'sample_size': 1, 'T': T, 'perturbation': perturbation,'swap_ratio':swap_ratio,
