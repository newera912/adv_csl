...
flags.DEFINE_integer('hidden2', 16, 'Number of units in hidden layer 2, P in our paper.')
...
flags.DEFINE_float('alpha_0', 2.0, 'prior of Beta distribution.')
flags.DEFINE_float('beta_0', 5.0, 'prior of Beta distribution.')
flags.DEFINE_float('p_encode', 0.0, 'trade off parameter of auto_encode.')
flags.DEFINE_float('p_kl', 0.0, 'trade off parameter of KL-divergence.')
flags.DEFINE_integer('KL_m', 10, 'approximate of the infinite sum in KLD.')
flags.DEFINE_integer('test_num', 600, 'test number of dataset.')
...
adj, features, adj_mask = load_data_1()
...
# adj = adj_train
adj_train = adj
y_train_belief, y_test_belief, y_train_un, y_test_un, train_mask, test_mask, omega_test = mask_test_edge_opinion(FLAGS.test_num, FLAGS.noise)
...
placeholders = {
    ...
    'adj_mask': tf.placeholder(tf.int32, shape=(y_train_un.shape[0], y_train_un.shape[0])),
...
with tf.name_scope('optimizer'):
    if model_str == 'gcn_ae':
        ...
        opt = OptimizerVAE(preds=model.reconstructions,
                           ...
                           mask=placeholders['labels_mask'], adj_mask=placeholders['adj_mask'], omega_t=placeholders['omega_t'],)
...
for epoch in range(FLAGS.epochs):
    ...
    feed_dict = construct_feed_dict(adj_norm, adj_label, features, placeholders, y_train_belief, y_train_un, train_mask, adj_mask, omega_test)
    ...
    if epoch < FLAGS.vae_epoch:
        ...
        outs = sess.run([opt.opt_op1, opt.cost1, opt.cost_belief, opt.cost_uncertain], feed_dict=feed_dict)
