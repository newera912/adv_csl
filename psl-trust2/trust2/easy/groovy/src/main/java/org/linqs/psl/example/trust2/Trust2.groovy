package org.linqs.psl.example.trust2;
...
public class Trust2 {
	...
	private class PSLConfig {
		...
		public PSLConfig(ConfigBundle cb) {
			...
			this.dbPath = cb.getString('experiment.dbpath', '/network/rit/lab/ceashpc/adil/psl_tmp2');
	...
	public Trust2(ConfigBundle cb) {
		log = LoggerFactory.getLogger(this.class);
		config = new PSLConfig(cb);
		ds = new RDBMSDataStore(new H2DatabaseDriver(Type.Disk, Paths.get(config.dbPath, 'trust2').toString(), true), cb);
	...
	public static ConfigBundle populateConfigBundle(String[] args) {
		ConfigBundle cb = ConfigManager.getManager().getBundle("trust2");
		if (args.length > 0) {
	...
	public static void main(String[] args) {
        ...
        Trust2 lp = new Trust2(configBundle);
		lp.run();
