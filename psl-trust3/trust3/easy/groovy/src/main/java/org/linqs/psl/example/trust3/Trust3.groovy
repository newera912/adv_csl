package org.linqs.psl.example.trust3;
...
public class Trust3 {
	...
	private class PSLConfig {
		...
		public PSLConfig(ConfigBundle cb) {
			...
			this.dbPath = cb.getString('experiment.dbpath', '/network/rit/lab/ceashpc/adil/psl_tmp3');
	...
	public Trust3(ConfigBundle cb) {
		log = LoggerFactory.getLogger(this.class);
		config = new PSLConfig(cb);
		ds = new RDBMSDataStore(new H2DatabaseDriver(Type.Disk, Paths.get(config.dbPath, 'Trust3').toString(), true), cb);
	...
	public static ConfigBundle populateConfigBundle(String[] args) {
		ConfigBundle cb = ConfigManager.getManager().getBundle("Trust3");
		if (args.length > 0) {
	...
	public static void main(String[] args) {
        ...
        Trust3 lp = new Trust3(configBundle);
		lp.run();
