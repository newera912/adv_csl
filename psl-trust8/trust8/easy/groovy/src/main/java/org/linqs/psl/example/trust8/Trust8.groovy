package org.linqs.psl.example.trust8;
...
public class Trust8 {
	...
	private class PSLConfig {
		...
		public PSLConfig(ConfigBundle cb) {
			...
			this.dbPath = cb.getString('experiment.dbpath', '/network/rit/lab/ceashpc/adil/psl_tmp8');
	...
	public Trust8(ConfigBundle cb) {
		log = LoggerFactory.getLogger(this.class);
		config = new PSLConfig(cb);
		ds = new RDBMSDataStore(new H2DatabaseDriver(Type.Disk, Paths.get(config.dbPath, 'Trust8').toString(), true), cb);
	...
	private void defineRules() {
        ...
        model.add(
		    //rule: ( Adjacent(P1,P2) & T(P1,P2) & Adjacent(P2,P3) & T(P2,P3) ) >> T(P1,P3),
	...
	public static ConfigBundle populateConfigBundle(String[] args) {
		ConfigBundle cb = ConfigManager.getManager().getBundle("Trust8");
		if (args.length > 0) {
	...
	public static void main(String[] args) {
        ...
        Trust8 lp = new Trust8(configBundle);
		lp.run();
