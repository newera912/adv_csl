package org.linqs.psl.example.sybils2;
...
public class Sybils2 {
	...
	private class PSLConfig {
		...
		public PSLConfig(ConfigBundle cb) {
			...
			this.dbPath = cb.getString('experiment.dbpath', '/network/rit/lab/ceashpc/adil/tmp/sybil2/');
	...
	public Sybils2(ConfigBundle cb) {
		log = LoggerFactory.getLogger(this.class);
		config = new PSLConfig(cb);
		ds = new RDBMSDataStore(new H2DatabaseDriver(Type.Disk, Paths.get(config.dbPath, 'Sybils2').toString(), true), cb);
	...
	public static ConfigBundle populateConfigBundle(String[] args) {
		ConfigBundle cb = ConfigManager.getManager().getBundle("Sybils2");
		if (args.length > 0) {
	...
	public static void main(String[] args) {
        ...
        Sybils2 lp = new Sybils2(configBundle);
		lp.run();
