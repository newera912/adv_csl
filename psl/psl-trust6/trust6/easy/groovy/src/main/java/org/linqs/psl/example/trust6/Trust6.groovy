package org.linqs.psl.example.trust6;
...
public class Trust6 {
	...
	private class PSLConfig {
		...
		public PSLConfig(ConfigBundle cb) {
			...
			this.dbPath = cb.getString('experiment.dbpath', '/network/rit/lab/ceashpc/adil/psl_tmp6');
	...
	public Trust6(ConfigBundle cb) {
		log = LoggerFactory.getLogger(this.class);
		config = new PSLConfig(cb);
		ds = new RDBMSDataStore(new H2DatabaseDriver(Type.Disk, Paths.get(config.dbPath, 'Trust6').toString(), true), cb);
	...
	public static ConfigBundle populateConfigBundle(String[] args) {
		ConfigBundle cb = ConfigManager.getManager().getBundle("Trust6");
		if (args.length > 0) {
	...
	public static void main(String[] args) {
        ...
        Trust6 lp = new Trust6(configBundle);
		lp.run();
