package org.linqs.psl.example.trust7;
...
public class Trust7 {
	...
	private class PSLConfig {
		...
		public boolean sqPotentials = false;
		...
		public PSLConfig(ConfigBundle cb) {
			...
			this.dbPath = cb.getString('experiment.dbpath', '/network/rit/lab/ceashpc/adil/psl_tmp7');
	...
	public Trust7(ConfigBundle cb) {
		log = LoggerFactory.getLogger(this.class);
		config = new PSLConfig(cb);
		ds = new RDBMSDataStore(new H2DatabaseDriver(Type.Disk, Paths.get(config.dbPath, 'Trust7').toString(), true), cb);
	...
	public static ConfigBundle populateConfigBundle(String[] args) {
		ConfigBundle cb = ConfigManager.getManager().getBundle("Trust7");
		if (args.length > 0) {
	...
	public static void main(String[] args) {
        ...
        Trust7 lp = new Trust7(configBundle);
		lp.run();
