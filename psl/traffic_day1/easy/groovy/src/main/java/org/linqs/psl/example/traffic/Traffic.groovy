package org.linqs.psl.example.traffic;
...
public class Traffic {
	...
	private class PSLConfig {
		...
		public PSLConfig(ConfigBundle cb) {
			...
			this.weightMap["Conjested"] = cb.getInteger('model.weights.conjested', weightMap["Conjested"]);
			this.weightMap["Nonconjested"] = cb.getInteger('model.weights.nonconjested', weightMap["Nonconjested"]);
            /*
	...
	public Traffic(ConfigBundle cb) {
		log = LoggerFactory.getLogger(this.class);
		config = new PSLConfig(cb);
		ds = new RDBMSDataStore(new H2DatabaseDriver(Type.Disk, Paths.get(config.dbPath, 'traffic_dc_day1').toString(), true), cb);
	...
	private void definePredicates() {
		...
		model.add predicate: "Conjested", types: [ConstantType.UniqueID];
		model.add predicate: "Nonconjested", types: [ConstantType.UniqueID];
	}
	...
	private void defineRules() {
		...
		model.add(
			rule: ( Adjacent(E1,E2) & Conjested(E2) ) >> Conjested(E1),
			squared: config.sqPotentials,
			weight : config.weightMap["Conjested"]
		);

            model.add(
			rule: ( Adjacent(E1,E2) & Nonconjested(E1) ) >> Nonconjested(E2),
			squared: config.sqPotentials,
			weight : config.weightMap["Nonconjested"]
		);

		model.add(
			rule: ( Nonconjested(E1) ) >> ~Conjested(E1),
			squared: config.sqPotentials,
			weight : config.weightMap["Nonconjested"]
		);
	...
	private void loadData(Partition obsPartition, Partition targetsPartition, Partition truthPartition) {
		...
		inserter = ds.getInserter(Conjested, obsPartition);
		InserterUtils.loadDelimitedData(inserter, Paths.get(config.dataPath, "conjested_obs.txt").toString());

		inserter = ds.getInserter(Nonconjested, obsPartition);
		InserterUtils.loadDelimitedData(inserter, Paths.get(config.dataPath, "nonconjested_obs.txt").toString());

		inserter = ds.getInserter(Conjested, targetsPartition);
		InserterUtils.loadDelimitedData(inserter, Paths.get(config.dataPath, "conjested_targets.txt").toString());

		inserter = ds.getInserter(Conjested, truthPartition);
		InserterUtils.loadDelimitedDataTruth(inserter, Paths.get(config.dataPath, "conjested_truth.txt").toString());
	}
	...
	private void runInference(Partition obsPartition, Partition targetsPartition) {
		...
		HashSet closed = new HashSet<StandardPredicate>([Adjacent, Nonconjested]);
	...
	private void writeOutput(Partition targetsPartition) {
		...
		PrintStream ps = new PrintStream(new File(Paths.get(config.outputPath, "conjested_infer.txt").toString()));
		AtomPrintStream aps = new DefaultAtomPrintStream(ps);
		Set atomSet = Queries.getAllAtoms(resultsDB,Conjested);
		for (Atom a : atomSet) {
			aps.printAtom(a);
		}
	...
	private void evalResults(Partition targetsPartition, Partition truthPartition) {
		Database resultsDB = ds.getDatabase(targetsPartition, [Conjested] as Set);
		Database truthDB = ds.getDatabase(truthPartition, [Conjested] as Set);
		...
		DiscretePredictionStatistics stats = dpc.compare(Conjested);
		double mse = cpc.compare(Conjested);
		log.info("MSE: {}", mse);
	...
	public static ConfigBundle populateConfigBundle(String[] args) {
		ConfigBundle cb = ConfigManager.getManager().getBundle("traffic");
		if (args.length > 0) {
	...
	public static void main(String[] args) {
        ...
        Traffic lp = new Traffic(configBundle);
		lp.run();
