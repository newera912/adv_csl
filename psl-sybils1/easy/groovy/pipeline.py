            ...
            for test_ratio in [0.1, 0.3, 0.4, 0.5][:]:
                for ratio_conflict in [0.0, 0.1, 0.2, 0.3, 0.4][:]:
                    for real_i in range(realizations)[:1]:
                        count += 1.0
                        f = dataroot + "sybils-attackedges-{}-T-{}-testratio-{}-conflictratio-{}-realization-{}-data-X.pkl".format(
                            attack_edge, T, test_ratio, ratio_conflict, real_i)
                        pkl_file = open(f, 'rb')
                        # V, E, Obs, E_X, X_b
                        ...
                        for window in range(T)[:]:
                            ...
                            result_file = str(attack_edge) + '_' + str(
                                test_ratio) + '_' + str(ratio_conflict) + '_' + str(window) + '_' + str(real_i) + '.txt'
                            ...
                            key = str(attack_edge) + '_' + str(test_ratio) + '_' + str(ratio_conflict) + '_' + str(
                                window) + '_' + str(real_i)
                            r_dict = {}
