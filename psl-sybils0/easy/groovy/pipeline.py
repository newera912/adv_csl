...
def result_analysis(sw_Omega,E_X):
    ...
    prob_mse, u_mse, prob_relative_mse, u_relative_mse, accuracy, recall_congested, recall_uncongested = calculate_measures(sw_Omega,Omega_X,E_X) 
    return prob_mse, u_mse, prob_relative_mse, u_relative_mse, accuracy, recall_congested, recall_uncongested 
 


def save_results(dataset,weekday,hour,refspeed,window,percent,prob_mse,u_mse,prob_relative_mse,u_relative_mse,accuracy,recall_congested,recall_uncongested,running_time):
    result = {'dataset':dataset, 'weekday':weekday, 'hour':hour, 'refspeed':refspeed, 'time_window':window, 'test_ratio': percent, 'prob_mse':prob_mse, 'u_mse':u_mse, 'prob_relative_mse':prob_relative_mse, 'u_relative_mse':u_relative_mse, 'accuracy':accuracy, 'recall_congested':recall_congested,'recall_uncongested':recall_uncongested, 'running_time':running_time}
                        ...
                        for real_i in range(realizations)[:1]:
                            ...
                            for window in range(T)[:]:
                                ...
                                result_file = str(attack_edge) + '_' + str(
                                    test_ratio)+ '_' + str(
                                    swap_ratio) + '_' + str(ratio_conflict) + '_'+ str(window) + '_' + str(real_i) + '.txt'
